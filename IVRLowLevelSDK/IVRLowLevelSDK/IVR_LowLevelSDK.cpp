...
{
    QFuture<uint> IVR_ShotThread = QtConcurrent::run([pIVR_CamType]()
    {
       uint Counter=0;
...
void IVRLowLevelSDK::IVR_RemoveVirtualCam(uint pIVR_CameraId)
{
    //A virtual Cam Collect everything that happens for a Render Camera in Unreal
    delete IVR_CameraPool[pIVR_CameraId];
    IVR_CameraPool[pIVR_CameraId]=nullptr;
    IVR_CameraPool.erase(IVR_CameraPool.begin() + pIVR_CameraId);
}

bool IVRLowLevelSDK::IVR_CheckRecordingState(uint pIVR_CameraId)
{
    QFuture<bool> IVR_ShotThread = QtConcurrent::run([pIVR_CameraId]()
    {
