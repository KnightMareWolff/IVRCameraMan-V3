...
AIVR_Camera::AIVR_Camera()
{
	...
	IVR_CameraCapture->SetupAttachment(IVR_Root);
	...
	IVR_CollisionSphere->SetupAttachment(IVR_Root);
...
// Called every frame
void AIVR_Camera::Tick(float DeltaTime)
{
	...
	case 0:	//Recording...Boa 
	{
		...
		if (!IVR_Enabled)
		{
			...
			AsyncTask(ENamedThreads::GameThread, [this]()
			{
				...
				OnBackBufferReadyToPresent = FSlateApplication::Get().GetRenderer()->OnBackBufferReadyToPresent().AddUObject(this, &AIVR_Camera::OnBackBufferReady);
...
void AIVR_Camera::IVR_RegisterCamera(FString CameraName, int32 LowLevelType, int32 LowLevelRecordingMode)
{
...
void AIVR_Camera::OnBackBufferReady(SWindow& SlateWindow, const FTexture2DRHIRef& BackBuffer)
{
...
bool AIVR_Camera::IVR_StartRecord()
{
...
bool AIVR_Camera::IVR_StopRecord()
{
	if (IVR_RecordingState != 0)
	{
		if (GEngine)
...
bool AIVR_Camera::IVR_CompileVideo()
{
	if (IVR_RecordingState == 0)
	{
		if (GEngine)
...
// Called when the game starts or when spawned
bool AIVR_Camera::IVR_LoadDefault()
{
...
// Called when the game starts or when spawned
bool AIVR_Camera::IVR_SetTransform(FTransform pTransform)
{
...
// Called when the game starts or when spawned
bool AIVR_Camera::IVR_SetRotation(FRotator    pRotation)
{
